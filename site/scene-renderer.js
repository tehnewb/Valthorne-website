...
export class BrowserSceneRenderer {
    ...
    render(projection,model,near,far){
        ...
        const w=h.renderWidth || h.canvasWidth || Math.max(1,Math.round(canvas.clientWidth));
        const height=h.renderHeight || h.canvasHeight || Math.max(1,Math.round(canvas.clientHeight));
