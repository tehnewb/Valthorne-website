...
export class BrowserNano {
    ...
    end(id){const c=this.get(id);if(c.dirty){this.host.graphics.composite(c.canvas);c.dirty=false;}}
    ...
    prepare(c){const {ctx,state:s}=c;ctx.save();ctx.resetTransform();ctx.scale(c.ratio||1,c.ratio||1);const base=ctx.getTransform();for(const clip of s.clips){ctx.setTransform(base.multiply(clip.matrix));ctx.beginPath();ctx.rect(clip.x,clip.y,clip.w,clip.h);ctx.clip();}ctx.setTransform(base.multiply(s.matrix));ctx.fillStyle=s.fill;ctx.strokeStyle=s.stroke;ctx.lineWidth=s.width;ctx.lineCap=s.cap;ctx.lineJoin=s.join;this.font(c);return ctx;}
