...
public final class Main implements Application {
    ...
    private void rebuild() {
        ...
        if(page>=0) {
            ...
            ui.layout();scroll.scrollY(saved);return;
        }
        ...
        copy.add(label("an open source Java game engine", width < 700 ? 14 : 17, MUTED)); row.add(copy);
    ...
    private NanoContainer card(int kind, String title, String body) {
        ...
        card.add(art); card.add(label(title, 30, WHITE)); card.add(label(body, width < 700 ? 12 : 13, MUTED)); card.add(button("Explore features  →", open)); return card;
    }
    private void buildFeature(NanoContainer content,float gutter) {
        exhibitSlot=null;
        var feature=FeaturePage.PAGES[page];
        var section=section(CLEAR,gutter,42);section.getLayout().gap(22);projects=section;about=section;
    ...
    @Override public void update(float delta) {
        ...
        if(pendingPage!=-2){page=pendingPage;pendingPage=-2;scroll.scrollY(0);rebuild();}
    ...
    @Override public void dispose() { if (ui != null) ui.dispose(); if (exhibit != null) exhibit.close(); }
}
