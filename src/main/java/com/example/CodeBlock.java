...
final class CodeBlock extends NanoPanel {
    ...
    CodeBlock(String code) {
        ...
        header.getLayout().widthPercent(100).row().itemsCenter();
        header.add(new NanoLabel("JAVA  /  EXAMPLE").fontSize(11).color(Main.MUTED));
        var space = new NanoContainer();
        space.getLayout().grow();
        header.add(space);
        ...
        int max = 0;
        for (var line : lines) max = Math.max(max, line.length());
        var viewport = new NanoScrollPanel().vertical(false).verticalBar(false).horizontal(true).horizontalBar(true);
        viewport.setStyle(NanoScrollPanel.BACKGROUND_COLOR_KEY, Main.CLEAR);
        viewport.setStyle(NanoScrollPanel.BORDER_WIDTH_KEY, 0f);
        viewport.getLayout().widthPercent(100).height(lines.length * 23 + 16).noShrink();
        var body = new NanoContainer();
        body.getLayout().column().width(Math.max(240, max * 8.5f)).noShrink();
        for (var line : lines) {
            var row = new NanoContainer();
            row.getLayout().row().height(23).noShrink();
            var matcher = TOKENS.matcher(line);
            var run = new StringBuilder();
            int runColor = 0xFFE2DFEB;
            while (matcher.find()) {
                ...
                if (run.length() > 0 && rgb != runColor) {
                    addRun(row, run.toString(), runColor);
                    run.setLength(0);
                }
                runColor = rgb;
                run.append(token);
    ...
    private static void addRun(NanoContainer row, String text, int color) {
        ...
        span.getLayout().width(text.length() * 8.4f).height(23).noShrink();
        row.add(span);
    }
}
