...
final class CodeBlock extends NanoPanel {
    ...
    private static final Set<String> KEYWORDS=Set.of("var","new","public","final","class","static","void","float","int","boolean","import","implements","true","false","return");
    CodeBlock(String code) {
        ...
        for(var line:lines) {
            ...
            while(matcher.find()) {
                ...
                var span=new NanoLabel(token).fontName("code").fontSize(14).color(new Color(rgb)).selectable(true);
                span.getLayout().width(token.length()*8.4f).height(23).noShrink();row.add(span);
            }
            body.add(row);
        }
