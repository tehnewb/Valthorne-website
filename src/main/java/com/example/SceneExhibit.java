...
final class SceneExhibit implements AutoCloseable {
    ...
    SceneExhibit() {
        background = surface(new Color(0xFF08070B));
        ...
        renderer.setEnvironmentIntensity(40);
        renderer.setExposure(1.1f);
        camera.setClipPlanes(.05f, 40);
        camera.setFieldOfViewDegrees(FOV);
        camera.setPosition(0, -DISTANCE, 0);
        camera.lookAt(0, 0, 0, 0, 0, 1);
    }

    private ModelInstance3D surface(Color color) {
        return new ModelInstance3D().setModel(plane).setRotation((float) Math.PI / 2, 0, 0).setMaterial(new Material3D().setTint(color).setRoughness(1).setMetallic(.35f).setCastsShadow(false).setReceivesShadow(false).setCullBackFaces(false));
    }
    ...
    void addSurface(UINode node) {
        var surface = surface(new Color(0xFF1A1721));
        nodes.add(node);
    ...
    void render(UINode slot, float scrollY, float time, boolean moving) {
        ...
        float dt = Math.max(0, Math.min(.05f, time - previousTime));
        previousTime = time;
