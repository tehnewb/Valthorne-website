...
final class Ornament extends NanoContainer {
    ...
    Ornament(int kind, DoubleSupplier clock) {
        this.kind = kind;
        this.clock = clock;
        setClickable(false);
    }
    ...
    public void draw(long vg) {
        ...
        if (kind == 0) {
            line(vg, x, cy, cx - 20, cy, 0xFF435044);
            line(vg, cx + 20, cy, x + w, cy, 0xFF435044);
            diamond(vg, cx, cy, 7, 0xFFE0BD7D);
            diamond(vg, cx, cy, 3, 0xFF55C8F5);
            return;
        }
        if (kind < 4) {
            float ix = x + 28;
            diamond(vg, ix, cy, 25, 0xFF91794E);
            diamond(vg, ix, cy, 19, 0xFF297AAA);
            if (kind == 1) {
                line(vg, ix - 12, cy + 7, ix, cy - 12, 0xFFE0BD7D);
                line(vg, ix, cy - 12, ix + 12, cy + 7, 0xFFE0BD7D);
                line(vg, ix - 12, cy + 7, ix + 12, cy + 7, 0xFFE0BD7D);
            } else if (kind == 2) {
                circle(vg, ix, cy, 10, 0xFFE0BD7D);
                line(vg, ix - 15, cy, ix + 15, cy, 0xFF55C8F5);
            } else {
                line(vg, ix - 10, cy - 9, ix - 10, cy + 9, 0xFFE0BD7D);
                line(vg, ix - 10, cy + 9, ix + 10, cy + 9, 0xFFE0BD7D);
                line(vg, ix + 10, cy + 9, ix + 10, cy - 9, 0xFFE0BD7D);
            }
            line(vg, ix + 44, cy, x + w, cy, 0xFF435044);
            return;
        }
        float r = Math.min(w * .42f, h * .42f), t = (float) clock.getAsDouble();
        circle(vg, cx, cy, r, 0xFF91794E);
        circle(vg, cx, cy, r - 7, 0xFF435044);
        circle(vg, cx, cy, r * .67f, 0xFF235273);
        for (int i = 0; i < 32; i++) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            line(vg, cx, cy, px, py, 0xFF235273);
            diamond(vg, px, py, 9, 0xFFE0BD7D);
            diamond(vg, px, py, 4, 0xFF55C8F5);
        }
        circle(vg, cx, cy, 35 + (float) Math.sin(t) * 3, 0xFF235273);
        diamond(vg, cx, cy, 29, 0xFFE0BD7D);
        diamond(vg, cx, cy, 21, 0xFF55C8F5);
        line(vg, cx, cy - 21, cx, cy + 21, 0xFF55C8F5);
        line(vg, cx - 21, cy, cx + 21, cy, 0xFF55C8F5);
    }
