...
final class PackedTree implements AutoCloseable {
        ...
        try (var resource = PackedTree.class.getResourceAsStream("/models/jacaranda/tree.vtr.gz")) {
            ...
            try (var in = new DataInputStream(new GZIPInputStream(resource))) {
                ...
                for (int part = 0; part < 3; part++) {
                    ...
                    if (count < 1 || count > 250000 || faces < 1 || faces > 100000)
                        throw new IOException("Invalid tree counts");
                    Vector3f[] p = new Vector3f[count], n = new Vector3f[count];
                    Vector2f[] uv = new Vector2f[count];
                    for (int i = 0; i < count; i++) {
                        p[i] = new Vector3f(in.readFloat(), in.readFloat(), in.readFloat());
                        n[i] = new Vector3f(in.readFloat(), in.readFloat(), in.readFloat());
                        uv[i] = new Vector2f(in.readFloat(), in.readFloat());
    ...
    @Override
    public void close() {for (var texture : textures) if (texture != null) texture.dispose();}
}
